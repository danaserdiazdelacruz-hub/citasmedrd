...
const server = createServer(async (req, res) => {
  ...
  // Telegram webhook
  // URL nueva: /webhook/telegram/:canalId  (canalId = canales_conectados.id)
  // URL legacy: /webhook/telegram (sin canalId) → 400 con instrucción.
  if (url.startsWith("/webhook/telegram") && method === "POST") {
    // Extraer canalId del path. Acepta querystring opcional.
    const match = url.match(/^\/webhook\/telegram\/([^/?#]+)/);
    const canalId = match?.[1];

    if (!canalId) {
      console.warn(`[telegram] URL sin canalId: ${url}`);
      respond(res, 400, {
        error: "missing canal_id in path",
        hint: "Configurar webhook con /webhook/telegram/<canal_conectado_id>",
      });
      return;
    }
    ...
    try {
      ...
      console.log(`[telegram] update recibido: id=${update.update_id} canal=${canalId.slice(0, 8)}`);

      // Telegram exige que respondamos rápido (timeout ~5s).
      // Enviamos 200 inmediato, procesamos en background.
      respond(res, 200, { ok: true });

      handleTelegramUpdate(canalId, update).catch(err => {
...
server.listen(ENV.PORT, HOST, async () => {
  ...
  console.log(`[startup] Webhook Telegram: /webhook/telegram/<canal_conectado_id>`);
  await smokeTestDb();
});
