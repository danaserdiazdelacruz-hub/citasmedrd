  ...
  try {
    if (update.message) {
      ...
      if (cmdEntity) {
        ...
        // Argumento del deep-link: `/start dra-carmen-vargas` → "dra-carmen-vargas"
        // Telegram pasa el payload como texto después del comando.
        const cmdArg = text.slice(cmdEntity.length).trim() || undefined;
        incoming = {
