...
export async function procesarMensaje(chatId: string, texto: string): Promise<void> {
  ...
  // Motivo: if we already have sede+nombre+telefono, anything the patient says IS the motivo
  if (!ses.motivo && ses.sede_id) {
    if (ses.nombre && ses.telefono) {
      // Ya le preguntamos el motivo, lo que escriba es el motivo
      ses.motivo = datos.motivo || texto.trim();
    } else if (datos.motivo) {
      ses.motivo = datos.motivo;
    } else {
      // Detect common medical words even if other data is missing
      const medTerms = ["bulto","bolto","dolor","sangrado","flujo","ardor","quiste","masa",
        "nódulo","nodulo","biopsia","chequeo","mama","mamas","seno","senos","brazo","cuello",
        "cabeza","pierna","espalda","pecho","abdomen","estomago","estómago","fiebre","mareo",
        "vomito","vómito","diarrea","tos","herida","fractura","infeccion","infección","alergia"];
      if (medTerms.some(t => tl.includes(t))) {
        ses.motivo = texto.trim();
      }
    }
  }
