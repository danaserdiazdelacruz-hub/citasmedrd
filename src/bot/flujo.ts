...
export async function procesarMensaje(chatId: string, texto: string): Promise<void> {
  ...
  // ── POSITIONAL DATA CAPTURE ──
  // If we have sede+telefono, figure out what's missing and assign accordingly
  if (ses.sede_id && ses.telefono) {
    const hasPhone = /\b(809|829|849)\d{7}\b/.test(texto);

    // Combined message: "daniela 8093456789. bulto mama"
    if (hasPhone && (!ses.nombre || !ses.motivo)) {
      const withoutPhone = texto.replace(/\b(809|829|849)\d{7}\b/, "").replace(/[+\d]/g, "");
      const parts = withoutPhone.split(/[.,;]+/).map(p => p.trim()).filter(p => p.length > 0);
      for (const part of parts) {
        if (!ses.nombre && part.length >= 3 && /^[a-záéíóúñü\s]+$/i.test(part)) {
          ses.nombre = part.split(/\s+/).map(w => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join(" ");
        } else if (!ses.motivo && ses.nombre) {
          ses.motivo = part;
        }
      }
    }

    // NOMBRE: if missing and text looks like a name (not a number, not a known command)
    if (!ses.nombre) {
      const cleaned = texto.replace(/\b(809|829|849)\d{7}\b/, "").replace(/[.,;]/g, " ").trim();
      // Split by "motivo" keyword if present
      const motivoParts = cleaned.split(/\bmotivo\s*(es|:)?\s*/i);
      const namePart = motivoParts[0]?.trim();
      const motivePart = motivoParts.length > 1 ? motivoParts[motivoParts.length - 1]?.trim() : null;

      if (namePart && namePart.length >= 3 && /^[a-záéíóúñü\s.]+$/i.test(namePart)) {
        ses.nombre = namePart.replace(/\s+/g, " ").split(" ")
          .map(w => w.replace(/\./g, ""))
          .filter(w => w.length > 0)
      ...
      if (motivePart && !ses.motivo) {
        ses.motivo = motivePart;
      }
    }

    // MOTIVO: if missing and we have nombre, anything patient says IS the motivo
    if (!ses.motivo && ses.nombre) {
      ses.motivo = datos.motivo || texto.trim();
    }
  }

  // Fallback: extract nombre from Haiku even before we have telefono
  if (!ses.nombre && ses.doctor_id && datos.nombre) {
    ses.nombre = datos.nombre;
  }

  // Fallback motivo: detect medical words even without nombre
  if (!ses.motivo && ses.sede_id && !ses.nombre) {
    if (datos.motivo) ses.motivo = datos.motivo;
  }
  ...
  // LAST RESORT: if we have everything EXCEPT servicio, force it
  if (ses.sede_id && !ses.servicio_id && ses.nombre && ses.telefono && ses.motivo) {
    const srvs = ses.servicios_disponibles ?? [];
    const srv = (ses.es_primera ? srvs.find(s => s.tipo === "primera_vez") : srvs.find(s => s.tipo === "normal"))
                ?? srvs[0];
    if (srv) {
      ses.servicio_id = srv.id;
      console.log(`[LAST RESORT] Servicio forzado: ${srv.nombre}`);
    } else {
      const freshSrvs = await buscarServicios(ses.sede_id);
      ses.servicios_disponibles = freshSrvs;
      if (freshSrvs[0]) {
        ses.servicio_id = freshSrvs[0].id;
        console.log(`[LAST RESORT] Servicios recargados: ${freshSrvs[0].nombre}`);
      }
    }
  }
  ...
  if (ses.sede_id && ses.servicio_id && ses.nombre && ses.telefono && ses.motivo
    ...
    console.log("[AUTO-TRIGGER] Buscando disponibilidad...");
  ...
  // ════════════════ CLAUDE — SOLO PARA PEDIR DATOS FALTANTES ════════════════
  ...
  console.log(`[CLAUDE] Pendientes: ${pendientes.join(", ") || "NINGUNO — BUG"}`);

  const systemPrompt = `Eres recepcionista de CitasMed RD. Máximo 2 líneas. Sin emojis.
Pide SOLO: ${pendientes.join(", ") || "nada"}.
PROHIBIDO: confirmar citas, inventar fechas/horarios/códigos, decir "cita registrada" o "nos comunicaremos". Solo pide lo que falta.`;
