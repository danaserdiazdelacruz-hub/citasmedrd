...
export async function exec_buscar_disponibilidad(args: { doctor_clinica_id: string; servicio_id: string }): Promise<string> {
  let servicioId = args.servicio_id;

  // Verificar que el servicio_id existe para este doctor_clinica
  const srvCheck = await supabase<any[]>("GET", "/rest/v1/servicios", null, {
    id: `eq.${servicioId}`, doctor_clinica_id: `eq.${args.doctor_clinica_id}`,
    select: "id", limit: "1",
  });

  // Si no existe, auto-detectar el primer servicio disponible
  if (!srvCheck.data?.length) {
    console.log(`[FIX] servicio_id inválido, buscando servicio real para dc=${args.doctor_clinica_id}`);
    const srvs = await supabase<any[]>("GET", "/rest/v1/servicios", null, {
      doctor_clinica_id: `eq.${args.doctor_clinica_id}`,
      activo: "eq.true", invisible_para_pacientes: "eq.false",
      select: "id,nombre,tipo", limit: "5",
    });
    const srv = srvs.data?.find((s: any) => s.tipo === "primera_vez") ?? srvs.data?.[0];
    if (!srv) return JSON.stringify({ dias: [], error: "No hay servicios configurados en esta sede" });
    servicioId = srv.id;
    console.log(`[FIX] Usando servicio: ${srv.nombre} (${servicioId})`);
  }
...
export async function exec_buscar_horarios(args: { doctor_clinica_id: string; servicio_id: string; fecha: string }): Promise<string> {
  ...
  // Auto-detectar servicio si el ID no es válido
  const srvCheck = await supabase<any[]>("GET", "/rest/v1/servicios", null, {
    id: `eq.${servicioId}`, doctor_clinica_id: `eq.${args.doctor_clinica_id}`,
    select: "id", limit: "1",
  });
...
export async function exec_agendar_cita(args: {
  ...
  if (!srvCheck.data?.length) {
    const srvs = await supabase<any[]>("GET", "/rest/v1/servicios", null, {
      doctor_clinica_id: `eq.${args.doctor_clinica_id}`,
      activo: "eq.true", select: "id,tipo", limit: "5",
    });
    const srv = srvs.data?.find((s: any) => s.tipo === "primera_vez") ?? srvs.data?.[0];
    if (srv) servicioId = srv.id;
  }
