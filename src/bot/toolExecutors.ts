...
export async function exec_buscar_doctor(args: { texto: string }): Promise<string> {
  ...
  // Método 2: nombre completo — requiere al menos 2 palabras (nombre Y apellido)
  const palabras = c.split(/\s+/).filter(w => w.length >= 2);
  if (palabras.length < 2) {
    return JSON.stringify({
      encontrado: false,
      mensaje: "Por favor indica el nombre completo del doctor (nombre y apellido). Por ejemplo: 'Hairol Pérez' o extensión '1006'.",
    });
  }

  // Buscar con nombre Y apellido presentes en el registro
  const r = await supabase<any[]>("GET", "/rest/v1/doctores", null, {
    activo: "eq.true",
  ...
  if (r.data?.length) {
    // Filtrar: el resultado debe contener TODAS las palabras entre nombre+apellido
  ...
  return JSON.stringify({ encontrado: false, mensaje: "No se encontró doctor con ese nombre. Por favor indica nombre y apellido completos, o usa el código de extensión." });
}
...
export async function exec_agendar_cita(args: {
  ...
  if (!cita.data?.[0]?.exito) {
    const errorMsg = cita.data?.[0]?.mensaje ?? "Horario no disponible.";
    console.log(`[AGENDAR] FALLÓ: ${errorMsg}`);
    return JSON.stringify({ exito: false, error: errorMsg });
  }
