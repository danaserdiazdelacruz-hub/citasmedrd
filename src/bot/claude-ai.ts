...
export async function interpretarTipoConsulta(texto: string): Promise<"primera" | "seguimiento" | null> {
  const r = await preguntarClaude(
    `Eres asistente médico. ¿Es primera vez o seguimiento? Responde SOLO: "primera" o "seguimiento". Si no puedes, responde "null".`,
    texto,
  );
...
export async function extraerNombre(texto: string): Promise<string | null> {
  const r = await preguntarClaude(
    `Extrae el nombre completo del texto. Responde SOLO el nombre. Si no hay nombre claro, responde "null".`,
    texto,
  );
...
export async function extraerTelefono(texto: string): Promise<string | null> {
  const r = await preguntarClaude(
    `Extrae el número de teléfono. Responde SOLO los dígitos. Si no hay teléfono, responde "null".`,
    texto,
  );
...
export async function interpretarConfirmacion(texto: string): Promise<"si" | "no" | null> {
  const r = await preguntarClaude(
    `¿El usuario confirma o cancela? Responde SOLO: "si" o "no". Si no puedes, responde "null".`,
    texto,
  );
  if (r === "si") return "si";
  if (r === "no") return "no";
...
export async function detectarIntencionCancelar(texto: string): Promise<boolean> {
  const r = await preguntarClaude(
    `¿El usuario quiere cancelar una cita existente? Responde SOLO: "si" o "no".`,
    texto,
  );
  return r === "si";
}
