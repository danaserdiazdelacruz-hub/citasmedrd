...
export const TOOL_DEFINITIONS = [
  ...
  {
    name: "agendar_cita",
    description: "Agenda una cita médica. SOLO úsala cuando tengas: doctor_clinica_id, horario (inicia_en), nombre del paciente y teléfono (10 dígitos). El motivo NO se pide — en su lugar pregunta si es primera_vez (true) o seguimiento (false). NUNCA la uses sin nombre y teléfono confirmados.",
    input_schema: {
      type: "object" as const,
      properties: {
        doctor_clinica_id: { type: "string", description: "UUID de la relación doctor-clínica" },
        ...
        nombre: { type: "string", description: "Nombre del paciente exactamente como lo dio. Si solo dio nombre de pila, úsalo así. NUNCA agregar la palabra Paciente." },
        telefono: { type: "string", description: "Teléfono (10 dígitos: 809/829/849 + 7 dígitos)" },
        primera_vez: { type: "boolean", description: "true si es su primera visita, false si ya ha venido antes. Preguntar sutilmente antes de agendar." },
        cedula: { type: "string", description: "OPCIONAL. Cédula dominicana formato 000-0000000-0 o 11 dígitos seguidos." },
      },
      required: ["doctor_clinica_id", "inicia_en", "nombre", "telefono"],
      additionalProperties: false,
    },
  },
  {
  ...
  {
    name: "reagendar_cita",
    description: "Cambia la fecha/hora de una cita existente SIN tener que cancelarla primero. Útil cuando el paciente quiere mover su cita a otro horario. El código mantiene igual, solo cambia cuándo será. Antes de usar esta herramienta, usa buscar_horarios para confirmar que el nuevo horario está disponible.",
    input_schema: {
      type: "object" as const,
      properties: {
        codigo: { type: "string", description: "Código de la cita existente (ej: CITA-ABC123)" },
        nuevo_inicia_en: { type: "string", description: "Nueva fecha+hora en ISO con offset -04:00 (ej: 2026-04-25T10:00:00-04:00)" },
      },
      required: ["codigo", "nuevo_inicia_en"],
      additionalProperties: false,
    },
  },
];
