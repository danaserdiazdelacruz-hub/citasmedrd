...
class TenantsRepo {
  ...
  /**
   * Carga un canal_conectado por su id, junto con su tenant.
   * Lo usa el webhook handler cuando la URL del webhook ya identifica el canal:
   *   POST /webhook/telegram/:canalId   →  el caller pasa canalId aquí.
   * Esto reemplaza el "agarrar el primer canal activo" que rompía multi-tenant.
   */
  async findCanalById(
    canalId: string
  ): Promise<{ tenant: Tenant; canal: CanalConectado } | null> {
