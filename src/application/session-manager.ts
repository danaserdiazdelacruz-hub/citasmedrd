...
export class SessionManager {
   ...
   * Si preserveMemoria=true, conserva paciente_telefono_conocido y datos del paciente.
   */
  async resetToIdle(sesionId: string, preserveMemoria = true): Promise<SesionConversacion> {
    if (preserveMemoria) {
      ...
      if (fresh) {
        ...
        const tel = fresh.contexto["paciente_telefono_conocido"];
        if (tel) memoria["paciente_telefono_conocido"] = tel;
        const nombre = fresh.contexto["paciente_nombre"];
        if (nombre) memoria["paciente_nombre_conocido"] = nombre;
        ...
        await db
          ...
          .eq("id", sesionId);
        const after = await this.loadFresh(sesionId);
        if (!after) throw new Error(`resetToIdle: sesión ${sesionId} desapareció`);
        return after;
      }
    }
