...
export function buildSystemPrompt(ctx: PromptContext): string {
  ...
  if (faq.atiende_ninos !== undefined) {
    // Generamos la línea de forma inequívoca según el caso, evitando
    // contradicciones tipo "NO atienden (edad 14 – sin tope)" que confunden al LLM.
    let lineaNinos: string;
    const min = faq.edad_minima;
    const max = faq.edad_maxima;
    if (faq.atiende_ninos) {
      if (min !== undefined && max !== undefined) {
        lineaNinos = `- Niños: SÍ atendemos pacientes de ${min} a ${max} años`;
      } else if (min !== undefined) {
        lineaNinos = `- Niños: SÍ atendemos a partir de ${min} años`;
      } else if (max !== undefined) {
        lineaNinos = `- Niños: SÍ atendemos hasta los ${max} años`;
      } else {
        lineaNinos = `- Niños: SÍ atendemos pacientes pediátricos`;
      }
    } else {
      // No atiende niños. Si hay edad_minima, usarla como info clave.
      if (min !== undefined) {
        lineaNinos = `- Edad mínima de atención: ${min} años — NO atendemos menores de esa edad`;
      } else {
        lineaNinos = `- Niños: NO atendemos pacientes pediátricos. Recomienda buscar un pediatra.`;
      }
    }
