...
// ─── Detección de saludo / small talk ─────────────────────────────────
// Si el texto es claramente un saludo o small talk, NO lo tratamos como
// búsqueda de doctor. En su lugar, mostramos el saludo de CitasMed.
...
function esSaludo(texto: string): boolean {
  ...
  // Match exacto contra la lista de saludos comunes
  return PALABRAS_SALUDO.some(p => limpio === p || limpio.startsWith(`${p} `));
}

// ─── Detección de intenciones generales (info, queja, ayuda) ──────────
// Si el texto contiene palabras clave de "preguntar info" / "ayuda" en vez
// de un nombre/teléfono concreto, dejamos que el LLM lo maneje.
...
function pareceIntencionGeneralNoBusqueda(texto: string): boolean {
  const limpio = texto.toLowerCase().trim();
  if (limpio.length === 0) return false;
...
/**
 * Decide si un texto en IDLE parece ser una búsqueda de doctor por
 * nombre/apellido/teléfono o una extensión.
 *
 * Heurística:
 *  - Si parece saludo o cortesía → NO es búsqueda
 *  - Si tiene pistas de intención general (cancelar, info, etc.) → NO es búsqueda
 *  - Si es muy corto (1-2 caracteres) → NO es búsqueda
 *  - Si es muy largo (>60 chars) → NO es búsqueda (probablemente es una pregunta)
 *  - Si es solo números (tipo teléfono o extensión) → SÍ es búsqueda
 *  - Si tiene palabras alfabéticas razonables → SÍ es búsqueda
 *
 * Devuelve true cuando intentar buscar tiene sentido.
 */
function pareceBusquedaDeDoctor(texto: string): boolean {
  ...
  // Solo dígitos (con espacios/guiones/paréntesis): seguro es búsqueda por teléfono/extensión
  const soloDigitos = limpio.replace(/[\s\-().+]/g, "");
  if (/^\d+$/.test(soloDigitos)) return true;

  // Tiene letras alfabéticas: probablemente nombre/apellido
  // (el usuario podría escribir "Dra. Carmen", "doctor pérez", "Vargas", etc.)
  const tieneLetras = /[a-záéíóúñ]{3,}/i.test(limpio);
  return tieneLetras;
}
...
/**
 * Lee el FAQ del PROFESIONAL específico desde `profesionales.configuracion.faq`.
 * Si el profesional no tiene FAQ propio, hace fallback al FAQ del tenant
 * (compat con tenants que ya tenían FAQ configurado antes de migración 007).
 *
 * Esta es la fuente de verdad para responder preguntas del paciente
 * (acepta_seguros, atiende_ninos, parqueo, etc.) según Spec v1.1 sec. 3.7.
 */
function faqDelProfesional(
  profesional: { configuracion?: Record<string, unknown> | null } | null,
  tenant: Tenant | null,
): Record<string, unknown> | undefined {
  // 1) Intentar FAQ propio del profesional
...
async function handleCommand(
  ...
  if (cmd === "start" || cmd === "menu") {
    ...
    // Deep-link: `/start <slug>` → paciente llega con doctor pre-identificado.
    // Buscamos al doctor por slug y mostramos saludo personalizado + menú.
    // Spec v1.1 sec. 4.2: el primer mensaje DEBE incluir disclaimer médico
    // (lo cubre saludoCitasMedConDoctor).
    const slug = msg.commandArg?.trim().toLowerCase();
    if (slug && /^[a-z0-9-]{2,80}$/.test(slug)) {
      const doctor = await profesionalesRepo.findBySlug(msg.tenantId, slug);
      if (doctor) {
        logInfo(ctx, `deep-link slug="${slug}" → doctor ${doctor.nombre} ${doctor.apellido}`);
        const fresh = await sessionManager.resetToIdle(sesion.id);
        const asistente = nombreAsistenteDe(tenant);
        const display = `${doctor.prefijo} ${doctor.nombre} ${doctor.apellido}`;
        // Guardamos el doctor en contexto (sin cambiar estado IDLE) para que el
        // flujo de agendar/info lo use sin volver a buscar.
        await sessionManager.transitionTo(fresh.id, "IDLE", {
          ...fresh.contexto,
        ...
        // Slug no resuelve a doctor — seguimos con saludo genérico
        logWarn(ctx, `deep-link slug="${slug}" no encontrado, saludo genérico`);
      }
    }
...
async function handleButton(
  ...
  try {
    switch (tipo) {
      ...
      case "agendar_con":return await handleAgendarConButton(msg, sesion, valor, ctx, tenant);
      case "info_doctor":return await handleInfoDoctorButton(msg, valor, ctx, tenant);
      case "buscar_otro": {
        // Usuario rechazó al doctor sugerido y quiere buscar otro.
        // Reset suave a IDLE preservando memoria + mostrar saludo de CitasMed
        // pero más corto (no es el primer mensaje).
        await sessionManager.resetToIdle(sesion.id, true);
        const t = tenant ?? await tenantsRepo.findById(msg.tenantId);
        const asistente = nombreAsistenteDe(t);
        return [{
          kind: "text",
 ...
 */
/**
 * Handler del primer turno conversacional: el usuario manda texto en IDLE
 * y queremos identificar al doctor por nombre/apellido/teléfono/extensión.
 *
 * Flujo:
 *  1. Si el texto parece teléfono (tras normalizar) → buscar por telefono/extensión.
 *  2. Si no, buscar por nombre/apellido.
 *  3. 1 match → mostrar confirmación con botones [✅ Sí, agendar | 🔍 Buscar otro]
 *  4. Varios matches → botones para que el usuario elija
 *  5. 0 matches → mensaje "no encuentro" pidiendo más info
 *  6. Si todo falla por algún motivo (DB error, etc.) → devolvemos null y
 *     dejamos que el caller use el LLM como fallback.
 *
 * Devuelve null cuando es mejor dejarle el turno al LLM (no se hizo nada útil).
 */
async function handleIdentificarDoctor(
  msg: IncomingMessage,
  ...
  // 1) Intentar como teléfono normalizado (E.164)
  let matchesPorTelefono: Array<{ id: string; prefijo: string; nombre: string; apellido: string; especialidad: string | null }> = [];
  const normalized = validatePhoneDO(texto);
  if (normalized.valid && normalized.normalized) {
    try {
      const res = await profesionalesRepo.buscarPorTelefonoOExtension(msg.tenantId, normalized.normalized);
      matchesPorTelefono = res.map(p => ({
        id: p.id, prefijo: p.prefijo, nombre: p.nombre, apellido: p.apellido,
        especialidad: p.especialidad,
      }));
    } catch (err) {
      logWarn(ctx, "búsqueda por teléfono falló", { err: String(err) });
    }
  }

  // 2) Intentar como extensión cortita (números puros sin formato de tel)
  let matchesPorExtension: typeof matchesPorTelefono = [];
  if (matchesPorTelefono.length === 0 && /^\d{1,6}$/.test(texto.replace(/\s/g, ""))) {
    try {
      const res = await profesionalesRepo.buscarPorTelefonoOExtension(msg.tenantId, texto.replace(/\s/g, ""));
      matchesPorExtension = res.map(p => ({
        id: p.id, prefijo: p.prefijo, nombre: p.nombre, apellido: p.apellido,
        especialidad: p.especialidad,
      }));
    } catch (err) {
      logWarn(ctx, "búsqueda por extensión falló", { err: String(err) });
    }
  }

  // 3) Si no es teléfono/extensión, buscar por nombre/apellido
  let matchesPorNombre: typeof matchesPorTelefono = [];
  const todoMatchesTel = matchesPorTelefono.length + matchesPorExtension.length;
  if (todoMatchesTel === 0) {
    // Limpiar prefijos comunes: "Dr.", "Dra.", "doctor", "doctora", "el", "la"
    const queryLimpio = texto
      .replace(/^(dr\.?|dra\.?|doctor|doctora|el\s+|la\s+)/gi, "")
      .trim();
    if (queryLimpio.length >= 2) {
      try {
        const res = await profesionalesRepo.buscarPorNombre(msg.tenantId, queryLimpio, 8);
        matchesPorNombre = res.map(p => ({
          id: p.id, prefijo: p.prefijo, nombre: p.nombre, apellido: p.apellido,
          especialidad: p.especialidad,
        }));
      } catch (err) {
        logWarn(ctx, "búsqueda por nombre falló", { err: String(err) });
  ...
  // 0 matches: mensaje de "no encuentro" + invitar a reintentar
  if (matches.length === 0) {
    logInfo(ctx, `identificación de doctor sin matches para "${texto}"`);
    // Si el texto fue muy corto o muy genérico, mejor dejar al LLM (puede ser
    // pregunta de info). Si fue largo y específico, mostramos "no encuentro".
    if (texto.length < 4) return null;
    return [{ kind: "text", text: M.doctorNoEncontrado(texto) }];
  }

  // 1 match: mostrar confirmación con botón
  if (matches.length === 1) {
    const p = matches[0];
    const display = `${p.prefijo} ${p.nombre} ${p.apellido}`;
    logInfo(ctx, `identificación de doctor: 1 match → ${display}`);
    return [{
      kind: "buttons",
  ...
  // Varios matches: botones para elegir
  logInfo(ctx, `identificación de doctor: ${matches.length} matches`);
  return [{
    kind: "buttons",
...
/**
 * Handler del botón "✅ Sí, agendar" (data: `agendar_con:<profesionalId>`).
 *
 * Es el atajo cuando el usuario confirma al doctor identificado en el primer
 * turno o cuando elige uno de varios matches. Hace los mismos chequeos que
 * `handleBusquedaProfesional` cuando hay 1 match (cita activa, etc.) y luego
 * delega en `mostrarSedesParaProfesional`.
 */
async function handleAgendarConButton(
  msg: IncomingMessage,
  sesion: SesionConversacion,
  ...
  const prof = await profesionalesRepo.findById(msg.tenantId, profesionalId);
  if (!prof) {
    logWarn(ctx, `agendar_con: profesional no encontrado: ${profesionalId}`);
    return [{ kind: "text", text: "Ese especialista ya no está disponible. Intenta de nuevo." }];
  }

  // Chequeo de cita activa antes de iniciar el flujo
  const telConocido = sesion.contexto["paciente_telefono_conocido"] as string | undefined;
  if (telConocido) {
    try {
      ...
      if (citas.length > 0) {
        const c = citas[0];
        logInfo(ctx, "agendar_con: paciente con cita activa", { codigo: c.codigo });
        return [{
          kind: "buttons",
          text: M.yaTienesCitaActiva(
            formatFechaHora(c.iniciaEn, ctx.tz),
  ...
  return await mostrarSedesParaProfesional(msg.tenantId, prof, sesion, ctx);
}


/**
 * Handler del botón "ℹ️ Información" (data: `info_doctor:<profesionalId>`).
 * Muestra una ficha con: especialidad, sedes, horarios, servicios, FAQ.
 * Spec v1.1 sec. 4.6.
 */
async function handleInfoDoctorButton(
  msg: IncomingMessage,
  profesionalId: string,
  ctx: LogCtx,
  _tenant: Tenant | null,
  ...
  const prof = await profesionalesRepo.findById(msg.tenantId, profesionalId);
  if (!prof) {
    logWarn(ctx, `info_doctor: profesional no encontrado: ${profesionalId}`);
    return [{ kind: "text", text: "No encuentro la información de ese especialista." }];
  }
  ...
  // Sedes
  try {
    const sedes = await profesionalesRepo.listarSedesPorProfesional(msg.tenantId, prof.id);
    if (sedes.length > 0) {
      partes.push("\n*📍 Sedes donde atiende:*");
    ...
    logWarn(ctx, "info_doctor: no pude listar sedes", { err: String(err) });
  }
...
async function handleText(
  ...
  // Otros estados con texto libre: si es IDLE, intentar identificar doctor primero
  if (sesion.estado === "IDLE") {
    // Si es un saludo o cortesía pequeña, mandamos el saludo de CitasMed
    if (esSaludo(msg.text!)) {
      const t = await tenantsRepo.findById(msg.tenantId);
      const asistente = nombreAsistenteDe(t);
      return [{ kind: "text", text: M.saludoCitasMed(asistente) }];
    }

    // Si parece búsqueda de doctor (nombre/apellido/teléfono/extensión),
    // intentamos identificar y mostrar la confirmación. Si NO matchea,
    // o si parece intención general (cancelar/info), caemos al LLM como antes.
    if (pareceBusquedaDeDoctor(msg.text!)) {
      const resultado = await handleIdentificarDoctor(msg, sesion, ctx, tenant);
      if (resultado !== null) return resultado;
      // Si devolvió null, no había match razonable y el handler decidió
      // que el LLM debe procesarlo (por ejemplo "ayuda con seguro" pasa
      // pareceBusquedaDeDoctor pero al buscar no matchea ningún doctor).
    }
...
async function handleIdleConLLM(
  ...
  try {
    ...
    const systemPrompt = buildSystemPrompt({
      ...
      // FAQ: por ahora usamos el del primer profesional como referencia (con
      // fallback al tenant si no tiene). Cuando haya UN doctor específico en
      // sesión (deep-link o tras identificación), usamos SU FAQ exclusivo.
      // Esto cumple Spec v1.1 sec 3.7 (FAQ por doctor).
...
// ─── Saludo principal de CitasMed ────────────────────────────────────
// Filosofía: María Salud es de CitasMed (la plataforma), no del consultorio.
// El usuario llega con un nombre/teléfono específico. El bot pregunta primero
// CON QUIÉN quiere agendar; el flujo de agendamiento real arranca después de
// identificar al doctor. Las funcionalidades de cancelar/consultar siguen
// disparándose por LLM-detect-intent (no requieren elegir doctor primero).
async function menuPrincipal(
  ...
  // (mantenemos esta UX: si llega alguien que ya tiene cita, le mostramos
  // los botones de consulta/cancelar como atajo).
      ...
      if (citas.length > 0) {
        ...
        return {
          ...
          text: M.saludoConCitaPendiente(
            tenant?.nombre_comercial ?? "CitasMed",
  ...
  // Saludo de CitasMed: pide nombre/apellido/teléfono del especialista.
  // Sin botones de menú — el usuario responde con texto.
  return {
    kind: "text",
    text: M.saludoCitasMed(asistente),
  };
}
