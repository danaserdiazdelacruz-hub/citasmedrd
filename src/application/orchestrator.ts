...
async function handleIdleConLLM(
  ...
  try {
    ...
    // ─── DIAGNÓSTICO TEMPORAL — quitar después de confirmar que el FAQ funciona ───
    const faqDebug = faqDelTenant(tenant);
    console.log("[DIAG-FAQ] tenant.id:", tenant?.id);
    console.log("[DIAG-FAQ] tenant.configuracion type:", typeof tenant?.configuracion);
    console.log("[DIAG-FAQ] tenant.configuracion keys:", tenant?.configuracion ? Object.keys(tenant.configuracion) : "null");
    console.log("[DIAG-FAQ] faqDelTenant result:", JSON.stringify(faqDebug));
    console.log("[DIAG-FAQ] systemPrompt incluye 'Seguros médicos':", systemPrompt.includes("Seguros médicos"));
    console.log("[DIAG-FAQ] systemPrompt incluye 'Senasa':", systemPrompt.includes("Senasa"));
    console.log("[DIAG-FAQ] systemPrompt length:", systemPrompt.length);
    // ─── FIN DIAGNÓSTICO ─────────────────────────────────────────────────────────────
