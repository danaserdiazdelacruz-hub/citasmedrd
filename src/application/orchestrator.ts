...
async function handleText(
  ...
  return [
    ...
    await menuPrincipal(msg.tenantId, sesion, tenant),
  ];
}
...
async function handleIdleConLLM(
  ...
  try {
    // Construir contexto real del tenant para el system prompt.
    // Limitamos las consultas a lo mínimo para no inflar el prompt.
    ...
    // Sedes y servicios: tomar los del primer profesional activo como representativos.
    // Si hay muchos profesionales con sedes/servicios distintos, ya es trabajo del bot
    // preguntar y filtrar — el prompt solo necesita pintar un panorama general.
    let sedesResumen: Array<{ nombre: string; ciudad?: string | null }> = [];
    let serviciosResumen: Array<{ nombre: string }> = [];
    if (profesionales.length > 0) {
      const primero = profesionales[0];
      const sedes = await profesionalesRepo.listarSedesPorProfesional(msg.tenantId, primero.id);
      sedesResumen = sedes.map(s => ({ nombre: s.sede.nombre, ciudad: s.sede.ciudad }));
      if (sedes.length > 0) {
        const servs = await profesionalesRepo.listarServiciosPublicos(sedes[0].profesionalSede.id);
        serviciosResumen = servs.slice(0, 10).map(s => ({ nombre: s.nombre }));
      }
    }
  ...
  if (intencionDetectada === "consultar") {
    ...
    if (tel) return await mostrarCitasDelPaciente(msg.tenantId, tel, "consultar", sesion, ctx);
    await sessionManager.transitionTo(sesion.id, "PIDIENDO_TELEFONO", { intencion: "consultar" });
    return [{ kind: "text", text: M.pidiendoTelefonoConsulta() }];
  }
  if (intencionDetectada === "cancelar") {
    const tel = sesion.contexto["paciente_telefono_conocido"] as string | undefined;
    if (tel) return await mostrarCitasDelPaciente(msg.tenantId, tel, "cancelar", sesion, ctx);
    await sessionManager.transitionTo(sesion.id, "PIDIENDO_TELEFONO", { intencion: "cancelar" });
    return [{ kind: "text", text: M.pidiendoTelefonoCancelar() }];
  }
