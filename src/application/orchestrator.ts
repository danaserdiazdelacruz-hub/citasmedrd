...
async function handleIdleConLLM(
  ...
  // El system prompt le pide saludar/conversar y luego invitar al menú.
  if (textoLLM.length > 0) {
    // Devolver texto del LLM, y si el texto NO incluye los botones, agregar
    // los botones como segundo mensaje para que el usuario pueda avanzar.
    const debeOfrecerMenu = !/(agendar|consultar|cancelar)/i.test(textoLLM)
      || textoLLM.length > 200;
    if (debeOfrecerMenu) {
      return [
    ...
    // El texto del LLM ya invita al menú → mandamos texto + menú compacto
    return [
      { kind: "text", text: textoLLM },
      await menuPrincipal(msg.tenantId, sesion, tenant),
    ];
  ...
  if (telConocido) {
    try {
      ...
      if (citas.length > 0) {
        ...
        return {
          kind: "buttons",
          text: M.saludoConCitaPendiente(nombre, formatFechaHora(c.iniciaEn, tz), c.servicioNombre),
