...
async function handleConfirmar(
  ...
  try {
    ...
    // Reset preservando memoria del paciente para próximas interacciones
    await sessionManager.resetToIdle(sesion.id, true);
    // Re-guardar el teléfono en memoria para que /menu siguiente lo use
    const fresh = await sessionManager.loadFresh(sesion.id);
    if (fresh) {
...
async function handlePidiendoTelefono(
  ...
  if (!phone.valid || !phone.normalized) {
    // Contar intentos inválidos. Después de 3, mostrar salida.
  ...
  await sessionManager.transitionTo(sesion.id, sesion.estado, {
    ...
    paciente_telefono_conocido: phone.normalized,    // memoria a largo plazo en sesión
...
async function handleIdleConLLM(
  ...
  // Sin intención clara: SOLO menú (no doble mensaje)
  return [await menuPrincipal(msg.tenantId, sesion)];
}
