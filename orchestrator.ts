...
import { runEffects, fxSend } from "./effects/runner.js";
...
import { mostrarCitasPorTelefono, mostrarCitasPorCodigo, pedirCodigoOTelefonoConsulta } from "./flows/consultar/index.js";
import { mostrarCitasCancelarPorTelefono, mostrarCitasCancelarPorCodigo, pedirCodigoOTelefonoCancelar, ejecutarCancelacion } from "./flows/cancelar/index.js";
...
async function route(
  ...
  switch (kind) {
    ...
    case "flow:consultar:mostrar": {
      ...
      if (tel) {
        const r = await mostrarCitasPorTelefono(ctx, tel);
        return r.effects as ReturnType<typeof fxSend>[];
      }
      const r = await pedirCodigoOTelefonoConsulta(ctx);
      return r.effects as ReturnType<typeof fxSend>[];
    }

    case "flow:consultar:por_codigo": {
      const r = await mostrarCitasPorCodigo(ctx, p["codigo"] as string);
      return r.effects as ReturnType<typeof fxSend>[];
    }
    ...
    case "flow:cancelar:mostrar": {
      const tel = sesion.contexto["paciente_telefono_conocido"] as string | undefined;
      if (tel) {
        const r = await mostrarCitasCancelarPorTelefono(ctx, tel);
        return r.effects as ReturnType<typeof fxSend>[];
      }
      const r = await pedirCodigoOTelefonoCancelar(ctx);
      return r.effects as ReturnType<typeof fxSend>[];
    }

    case "flow:cancelar:por_codigo": {
      const r = await mostrarCitasCancelarPorCodigo(ctx, p["codigo"] as string);
      return r.effects as ReturnType<typeof fxSend>[];
    }
...
async function interceptDelegation(
  ...
  if (intencion === "consultar") {
    const r = await mostrarCitasPorTelefono(ctx, telefono);
    const { messages: ms } = await runEffects(r.effects);
    delegados = ms;
  } else if (intencion === "cancelar") {
    const r = await mostrarCitasCancelarPorTelefono(ctx, telefono);
    const { messages: ms } = await runEffects(r.effects);
    delegados = ms;
  }
